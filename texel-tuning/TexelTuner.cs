...
public static class TexelTuner
{
        ...
        return new List<IntParameter>
    {
        ...
        // Bishop pair
        new() { Name = nameof(EvalWeights.BishopPairMg), Get = w => w.BishopPairMg, Set = (w, v) => w.BishopPairMg = v, Min = 0,   Max = 60 },
        new() { Name = nameof(EvalWeights.BishopPairEg), Get = w => w.BishopPairEg, Set = (w, v) => w.BishopPairEg = v, Min = 0,   Max = 80 },

        // Knight mobility
        new() { Name = nameof(EvalWeights.KnightMobMg), Get = w => w.KnightMobMg, Set = (w, v) => w.KnightMobMg = v, Min = 0, Max = 12 },
        new() { Name = nameof(EvalWeights.KnightMobEg), Get = w => w.KnightMobEg, Set = (w, v) => w.KnightMobEg = v, Min = 0, Max = 12 },

        // Bishop mobility
        new() { Name = nameof(EvalWeights.BishopMobMg), Get = w => w.BishopMobMg, Set = (w, v) => w.BishopMobMg = v, Min = 0, Max = 12 },
        new() { Name = nameof(EvalWeights.BishopMobEg), Get = w => w.BishopMobEg, Set = (w, v) => w.BishopMobEg = v, Min = 0, Max = 12 },
        ...
        // Rook on open file
        new() { Name = nameof(EvalWeights.RookOpenMg), Get = w => w.RookOpenMg, Set = (w, v) => w.RookOpenMg = v, Min = 0, Max = 80 },
        new() { Name = nameof(EvalWeights.RookOpenEg), Get = w => w.RookOpenEg, Set = (w, v) => w.RookOpenEg = v, Min = 0, Max = 40 },

        // Isolated pawn (negative = penalty)
        ...
        // King safety (positive = penalty for having open file near king)
        new() { Name = nameof(EvalWeights.KingOwnOpenMg),         Get = w => w.KingOwnOpenMg,         Set = (w, v) => w.KingOwnOpenMg = v,         Min = 0, Max = 100 },
        new() { Name = nameof(EvalWeights.KingOwnSemiOpenMg),     Get = w => w.KingOwnSemiOpenMg,     Set = (w, v) => w.KingOwnSemiOpenMg = v,     Min = 0, Max = 50 },
        new() { Name = nameof(EvalWeights.KingAdjacentOpenMg),    Get = w => w.KingAdjacentOpenMg,    Set = (w, v) => w.KingAdjacentOpenMg = v,    Min = 0, Max = 60 },
        new() { Name = nameof(EvalWeights.KingAdjacentSemiOpenMg),Get = w => w.KingAdjacentSemiOpenMg,Set = (w, v) => w.KingAdjacentSemiOpenMg = v,Min = 0, Max = 40 },
...
private static List<IntParameter> BuildParameterList()
{
    ...
    {
        int r = rank; 
        parameters.Add(new() { Name = $"PassedMg[{r}]", Get = w => w.PassedMg[r], Set = (w, v) => w.PassedMg[r] = v, Min = 0, Max = 150 });
